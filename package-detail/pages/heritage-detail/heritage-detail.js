const { getHeritageById, getRecommendations } = require('../../data/heritages-full');
...
Page({
  ...
  playVideo() {
    ...
    wx.navigateTo({
      url: `/package-detail/pages/heritage-video/heritage-video?url=${encodeURIComponent(video.url)}&title=${encodeURIComponent(video.title || this.data.heritage.name)}`
    });
  },
  ...
  goDetail(e) {
    const { id } = e.currentTarget.dataset;
    wx.redirectTo({ url: `/package-detail/pages/heritage-detail/heritage-detail?id=${id}` });
  },
  ...
  onShareAppMessage() {
    ...
    return {
      ...
      path: `/package-detail/pages/heritage-detail/heritage-detail?id=${heritage.id}`,
