/**
 * AI 助手 — 云函数 / 后端 / 直连 SiliconFlow
 */
const { resolveTransportMode } = require('./ai-transport.js');
const backendClient = require('./ai-backend-client.js');
...
function chat(message, userContext, locale, options) {
  ...
  if (resolveTransportMode() === 'backend') {
    return backendClient.chat({
      message: text,
      imageBase64: base64,
      mimeType: (options && options.mimeType) || 'image/jpeg',
      userContext,
      locale: loc,
      intent: options && options.intent
    });
  }
...
function planRoute(payload, userContext, locale) {
  if (resolveTransportMode() === 'backend') {
    return backendClient.planRoute({
      ...(payload || {}),
      userContext,
      locale: locale || 'zh-CN'
    });
...
function recognize(imageBase64, mimeType, userContext, locale) {
  if (resolveTransportMode() === 'backend') {
    return backendClient.recognize({
      imageBase64,
